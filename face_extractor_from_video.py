## Developed by Saptarshi Sanyal
## The following code will extract human face from video live feed data.
...
while (True):
    ...
    for (x, y, w, h) in faces:
        ...
        #FaceFileName = "cropped_face/" + datetime.utcnow().strftime('%y-%m-%d %H:%M:%S.%f')[:-3] + ".jpg"  # folder path and random name image
        fname = "e:/cropped_face/"  + datetime.utcnow().strftime('%y%m%d%H%M%S%f')[:-2] + ".jpg"
        print(fname)
        #cv2.imwrite(FaceFileName, sub_face)
        cv2.imwrite(fname, sub_face)
